...
class Summers_Snenko
{
    ...
    public static function getMeta_description($use=null, $use1=null){
        $page = Zend_Controller_Front::getInstance()->getRequest()->getControllerName();
        $all = require(APPLICATION_PATH . '/configs/description.php');
        $description =       array_merge(
            $all['default'],
            ($all[$page])?$all[$page]:array(),
            ($use)?$use:array(),
            ($use)?$use1:array()
        );
        ...
        foreach($description as $k=>$v){
            $description[$k] = $translate->_($v);
        }

        return implode(',', $description);
    }
    /**
     * отримуємо keywords для конкретних сторінок
     * @param $page
     *
     * @return string
     */
    public static function getMeta_keywords($use_keywords=null, $use_keywords1=null){
        if($use_keywords && !is_array($use_keywords))
            $use_keywords = explode(',', $use_keywords);

        $page = Zend_Controller_Front::getInstance()->getRequest()->getControllerName();//->getActionName();
        $all = require(APPLICATION_PATH . '/configs/keywords.php');
        $keywords = array_merge(
            ($use_keywords1)?$use_keywords1:array(),
            ($use_keywords)?$use_keywords:array(),
            $all['default'],
            ($all[$page])?$all[$page]:array()
        );
